...
import { useCreateAssetGroup } from "../../hooks/useCampaignManagement";
import { toast } from "sonner";
import { CheckCircle2, XCircle, Info, Upload } from "lucide-react";

interface AssetGroupBuilderProps {
  clientId: number;
  open: boolean;
...
export function AssetGroupBuilder({
  ...
  open,
  onOpenChange,
  campaignId = "camp_mock_123", // default for standalone builder
  ...
  const createMutation = useCreateAssetGroup(clientId);

  // Validation
  const hasLogo = logos.filter(Boolean).length >= 1;
  const hasImage = images.filter(Boolean).length >= 1;
  const hasHeadlines = headlines.filter(h => h.trim()).length >= 3;
  const hasLongHeadline = longHeadlines.filter(h => h.trim()).length >= 1;
  const hasDescriptions = descriptions.filter(d => d.trim()).length >= 2;
  ...
  const handlePublish = (e: React.FormEvent) => {
    ...
    headlines.filter(h => h.trim()).forEach(text => payloadAssets.push({ fieldType: "HEADLINE", text }));
    longHeadlines.filter(h => h.trim()).forEach(text => payloadAssets.push({ fieldType: "LONG_HEADLINE", text }));
    descriptions.filter(d => d.trim()).forEach(text => payloadAssets.push({ fieldType: "DESCRIPTION", text }));
    images.filter(Boolean).forEach(url => payloadAssets.push({ fieldType: "MARKETING_IMAGE", url }));
    logos.filter(Boolean).forEach(url => payloadAssets.push({ fieldType: "LOGO", url }));
    videos.filter(Boolean).forEach(url => payloadAssets.push({ fieldType: "YOUTUBE_VIDEO", url }));
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col p-0">
        <div className="px-6 py-4 border-b border-slate-200">
          <DialogTitle className="text-xl font-semibold">Build Asset Group</DialogTitle>
          <p className="text-sm text-slate-500 mt-1">
            Provide texts, images, and videos. Google will mix and match these to create ads.
          </p>
        </div>
        ...
        <div className="flex-1 overflow-auto flex">
          ...
          <div className="flex-1 p-6 space-y-8">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., Summer Promo" />
                </div>
                <div className="space-y-2">
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-medium text-slate-900 border-b pb-2">Images & Logos</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
            ...
            <div className="space-y-4">
              ...
              {headlines.map((val, i) => (
                <Input key={i} value={val} onChange={(e) => updateList(i, e.target.value, headlines, setHeadlines)} placeholder="Headline (max 30 chars)" maxLength={30} />
              ))}
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-medium text-slate-900 border-b pb-2 flex justify-between">
                ...
                <Button variant="ghost" size="sm" onClick={() => addField(longHeadlines, setLongHeadlines)} className="h-6 px-2 text-xs">Add Long Headline</Button>
              </h3>
              {longHeadlines.map((val, i) => (
                <Input key={i} value={val} onChange={(e) => updateList(i, e.target.value, longHeadlines, setLongHeadlines)} placeholder="Long Headline (max 90 chars)" maxLength={90} />
              ))}
            </div>
          ...
          <div className="w-72 bg-slate-50 border-l border-slate-200 p-6 flex flex-col">
            ...
            <div className="space-y-3 flex-1">
              <ReqItem met={hasName} label="Asset Group Name" />
              <ReqItem met={hasFinalUrl} label="Final URL" />
              <ReqItem met={hasLogo} label="At least 1 Logo" />
              <ReqItem met={hasImage} label="At least 1 Image" />
              <ReqItem met={hasHeadlines} label={`Headlines (${headlines.filter(h => h.trim()).length}/3)`} />
              <ReqItem met={hasLongHeadline} label="At least 1 Long Headline" />
              <ReqItem met={hasDescriptions} label={`Descriptions (${descriptions.filter(d => d.trim()).length}/2)`} />
            </div>
            ...
            <div className="pt-4 border-t border-slate-200 mt-6">
              ...
              <p className="text-xs text-slate-500 mt-1">{isValid ? "Excellent - Ready to publish" : "Incomplete - Provide more assets"}</p>
            </div>
          </div>
        </div>
        ...
        <DialogFooter className="px-6 py-4 border-t border-slate-200 bg-slate-50">
          ...
          >
            {createMutation.isPending ? "Publishing..." : "Publish Asset Group"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
